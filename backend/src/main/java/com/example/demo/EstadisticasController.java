...
public class EstadisticasController {
    ...
    // ObjectMapper: Spring Boot auto-configures one — use @Autowired with qualifier
    ...
    public ResponseEntity<?> usuarioTop() {
        if (mongoTemplate == null)
            return ResponseEntity.ok("MongoDB no disponible");
        ...
        return ResponseEntity
                .ok(results.getMappedResults().isEmpty() ? "No hay datos" : results.getMappedResults().get(0));
    }
    ...
    public ResponseEntity<?> tipoContinenteTop() {
        ...
        var agg = org.springframework.data.mongodb.core.aggregation.Aggregation.newAggregation(
                ...
                org.springframework.data.mongodb.core.aggregation.Aggregation.group("continentGanador").count()
                        .as("victorias"),
                org.springframework.data.mongodb.core.aggregation.Aggregation.sort(
                        org.springframework.data.domain.Sort.Direction.DESC, "victorias"),
    ...
    public ResponseEntity<?> statsGlobales() {
        ...
        long total = 0;
        try {
            total = partidaMongoRepository.count();
        } catch (Exception e) {
        }
        ...
        if (mongoTemplate != null) {
            try {
                var aggCont = org.springframework.data.mongodb.core.aggregation.Aggregation.newAggregation(
                        ...
                        org.springframework.data.mongodb.core.aggregation.Aggregation.group("continentGanador").count()
                                .as("count"));
                porContinente.addAll(
                        mongoTemplate.aggregate(aggCont, "partidas_stats", java.util.Map.class).getMappedResults());
            } catch (Exception e) {
            }
        }
