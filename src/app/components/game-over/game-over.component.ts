...
@Component({
  ...
  template: `
    <div class="go-wrap">
      ...
      <div class="go-card">
        ...
        <div class="v-pre">VICTORIA TÁCTICA</div>
        <h1 class="w-name" [style.color]="wf()?.color">{{ game.winner() }}</h1>
        <div class="w-icon" [innerHTML]="wf()?.svgIcon"></div>
        <p class="w-lore">{{ wf()?.lore }}</p>
        <div class="stats">
          <div class="s-row"><span>⏱️ Duración</span><span class="v">{{ game.turnNumber() - 1 }} turnos</span></div>
          <div class="s-row"><span>🚀 Misiles</span><span class="v">{{ tmu() }}</span></div>
        ...
        <div class="replay">
          <div class="log-list">
            @for (e of game.battleLog().slice().reverse(); track $index) {
              <div class="l-row" [class]="'log-' + e.type"><span class="t">T{{ e.turn }}</span><span>{{ e.abilityIcon }}</span><span>{{ e.message }}</span></div>
            }
          </div>
        </div>
  ...
  styles: [`
    ...
    .replay { width: 100%; }
    .log-list { max-height: 180px; overflow-y: auto; display: flex; flex-direction: column; gap: 4px; padding-right: 5px; }
    ...
    .l-row .t { color: rgba(255,255,255,0.3); font-weight: 800; min-width: 25px; }
...
export class GameOverComponent {
  ...
  tmu = computed(() => this.game.battleLog().filter(e => ['missile','snipe','burst','aoe','drain'].includes(e.type)).length * 5);
